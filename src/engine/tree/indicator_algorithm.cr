...
module AlgoBacktester::Tree
  ...
  class SMA < IndicatorAlgorithm
    ...
    def run(strategy : AbstractStrategy) : {Bool, AlgorithmError?}
      ...
      return {false, AlgorithmError.new("Indicator failed because selected period cannot be negative")} if @period < 0

      talib_result = CrystalTalib.execute(
        name: "SMA",
        start_idx: 0,
      ...
      return {false, AlgorithmError.new("Indicator failed because selected period be > #{sma_values.size}")} if @period > sma_values.size
